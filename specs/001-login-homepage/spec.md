...
- Q: OAuth 2.0 流程類型 → A: Authorization Code Flow (with PKCE) - 前端取得授權碼，後端交換 token（最安全，業界最佳實踐）
- Q: 會話 Token 儲存策略 → A: 資料庫儲存（可擴展）- 會話資料存在資料庫，支援即時撤銷和審計（推薦用於多裝置）
- Q: Email 唯一性規則 → A: Email 全域唯一 - 一個 email 只能對應一個使用者，多個社群帳號可連結同一使用者
- Q: 個資保護與合規要求 → A: 基本資料保護 - 支援帳號刪除、資料加密、明確告知收集用途（推薦 MVP）
- Q: 審計日誌保留策略 → A: 90 天保留 - 中期保留，平衡安全追蹤需求與儲存成本（推薦 MVP）
...
- **網路異常**：當社群登入服務（Google/Facebook）無回應或超時時，系統應如何處理？
  ...
  - 錯誤處理：回傳 HTTP 503 Service Unavailable，錯誤訊息：「社群登入服務暫時無法使用，請稍後再試」
...
- **帳號刪除**：當使用者刪除帳號後，如何處理審計日誌中的個人識別資訊？如何確保資料不可恢復？

## 功能需求 *(必填)*

### 功能需求

- **FR-001**: 系統必須支援 Google OAuth 2.0 登入流程（使用 Authorization Code Flow with PKCE），接受前端提供的授權碼（authorization code），向 Google 交換 access token 並驗證其有效性
- **FR-002**: 系統必須支援 Facebook Login 登入流程（使用 Authorization Code Flow with PKCE），接受前端提供的授權碼，向 Facebook 交換 access token 並驗證其有效性
...
- **FR-008**: 系統必須能夠將相同 email 的不同社群帳號連結到同一使用者。每個 email 在系統中必須是全域唯一的，當使用者用新的社群帳號（但相同 email）登入時，自動連結到現有使用者而非建立新帳號。當使用者用新社群帳號登入，且該帳號的 email 已存在於系統中：
  - 如果 email 屬於當前使用者的其他社群帳號 → 自動連結
  - 如果 email 屬於不同使用者 → 拒絕連結並回傳錯誤 (HTTP 409 Conflict)，錯誤訊息：「此電子郵件已被其他帳號使用，請使用其他方式登入」
...
- **FR-013**: 系統必須支援 token 更新機制（refresh token），避免使用者頻繁重新登入。Refresh Token 詳細規範：
  - Access Token 過期時間：30 天（如 FR-012）
  - Refresh Token 過期時間：90 天
  - Token Rotation 策略：每次使用 refresh token 更新時，產生新的 access token 與 refresh token；舊的 refresh token 立即失效（一次性使用）
  - Revoke 邏輯：使用者登出時，同時撤銷 access token 與 refresh token；偵測到 refresh token 重複使用時（可能的 token 洩漏），撤銷該使用者的所有 sessions
- **FR-014**: 系統必須提供帳號刪除功能，當使用者刪除帳號時，永久移除所有個人資料（使用者資訊、社群帳號連結、會話記錄），但保留匿名化的審計日誌以符合安全追蹤需求。審計日誌匿名化規則：
  - 保留欄位：id, action, timestamp, result, error_message
  - 匿名化欄位：user_id 設為 NULL
  - 刪除欄位：ip_address（完全移除，即使已加密）
  - 匿名化後的日誌保留 90 天，之後自動清理
- **FR-015**: 系統必須在首次登入時明確告知使用者資料收集的用途（提供登入服務、個人化體驗），並確保敏感資料（如 IP 位址）經過適當加密儲存
- **FR-016**: 系統必須實施審計日誌自動清理機制，將超過 90 天的日誌記錄自動永久刪除，以控制儲存成本並符合資料保留政策

### 關鍵實體

- **使用者 (User)**: 代表應用程式的使用者帳號。屬性包括：唯一識別碼、email（全域唯一）、姓名、頭像 URL、建立時間、最後登入時間。
- **社群帳號連結 (SocialAccount)**: 代表使用者連結的社群登入帳號。屬性包括：提供者類型（Google/Facebook）、提供者使用者 ID、連結時間。一個使用者可以連結多個社群帳號。
- **會話 (Session)**: 代表使用者的登入會話，儲存在資料庫中以支援即時撤銷和多裝置管理。屬性包括：會話 token、使用者識別碼、建立時間、過期時間、裝置資訊（選填）、是否已撤銷。
- **審計日誌 (AuditLog)**: 記錄所有安全相關操作，保留 90 天後自動清理。屬性包括：操作類型（登入/登出/驗證失敗）、使用者識別碼、時間戳記、IP 位址（加密）、結果（成功/失敗）、錯誤訊息（如有）。
